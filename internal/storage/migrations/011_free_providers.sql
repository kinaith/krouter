...
CREATE TABLE IF NOT EXISTS free_provider_state (
    ...
    -- JSON array of {protocol, krouter_provider_name, key_setup_hint} entries
    -- for vendors that expose more than one protocol endpoint (e.g. OpenRouter
    -- offers both OpenAI-compatible /chat/completions and Anthropic-compatible
    -- /messages on the same API key). Routing reads this so anthropic-protocol
    -- requests can also benefit from the free credit. Empty array '[]' means
    -- the provider speaks only the primary protocol column above.
    additional_protocols_json   TEXT NOT NULL DEFAULT '[]',
